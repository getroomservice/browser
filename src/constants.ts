// export const WS_URL = 'ws://localhost:3452';
// export const DOCS_URL = 'http://localhost:3454/docs';
// export const PRESENCE_URL = 'http://localhost:3454/presence';

// export const WS_URL = 'wss://super.roomservice.dev/ws';
// export const DOCS_URL = 'https://super.roomservice.dev/docs';
// export const PRESENCE_URL = 'https://super.roomservice.dev/presence';

export const WS_URL = 'wss://super.stagingmcstagingface.com/ws';
export const DOCS_URL = 'https://super.stagingmcstagingface.com/docs';
export const PRESENCE_URL = 'https://super.stagingmcstagingface.com/presence';
